...
std::vector<Point> getAStarPath(std::vector<std::vector<int>> map, Point start, Point end) {   // in map index 0 means not visited, 1 blocked, 2 visited
	...
	for (int i = 0; i < map.size(); i++) {
		cost_so_far[i].resize(map[i].size());
	}
	...
	while (!queue.empty()) {
		...
		for (int i = -1; i <= 1; i++) {
			for (int j = -1; j <= 1; j++) {
				if (i != 0 || i != 0) {
					double new_cost = cost_so_far[current.x][current.y] + sqrt(i*i + j*j);
					if (map[current.x + i][current.y + j] == 0 && (new_cost < cost_so_far[current.x + i][current.y + j] || abs(cost_so_far[current.x + i][current.y + j]) < 1e-9)) {
	...
	while (answer[answer.size() - 1] != start) {
